...
@property (nonatomic, copy) void(^singleTapHandler)(void);
@end
...
- (void)switchNavigationBar {
    if (self.navigationBar.isHidden) {
        ...
        CGFloat ty = 0;
        if(self.navigationBar.hz_orientation == HZYImageScanViewNavigationVertical) {
            ty = XX_NAVIGATION_HEIGHT;
        } else {
            ty = 64;
        }
        [UIView animateWithDuration:.25 animations:^{
            self.navigationBar.transform = CGAffineTransformMakeTranslation(0, ty);
        }];
    }else{
        [UIView animateWithDuration:.25 animations:^{
...
- (UIImageView *)imageViewWithIndex:(NSUInteger)index {
    ...
    if (self.thumbArray.count > 0) {
        if ([self.thumbArray[index] isKindOfClass:[UIImage class]]) {
            ...
            [imageView xx_imageWithUrl:self.thumbArray[index]];
        }
    }else{
        if ([self.imageArray[index] isKindOfClass:[UIImage class]]) {
            ...
            imageView = [UIImageView new];
            [imageView xx_imageWithUrl:self.imageArray[index]];
        }
    }
    return imageView;
}
...
- (void)configNavigationBar {
    ...
    navi.showDeleteBtn = self.deletable;
...
- (void)setUrl:(NSString *)url {
    ...
    UIImage *image = [UIImage xx_imageWithFileName:url];
...
- (void)setThumbUrl:(NSString *)thumbUrl {
    self.thumbImage = [UIImage xx_imageWithFileName:thumbUrl];
}
